...
#Building the Seq2seq Architecture

#Create a placeholder for inputs and outputs- Defining input parameters to the encoder model
def model_inputs():
    inputs=tf.placeholder(tf.int32,[None,None],name="input")
    output=tf.placeholder(tf.int32,[None,None],name="output")
    lr=tf.placeholder(tf.float32,name="learning rate")
    keep_prob=tf.placeholder(tf.float32,name="keep_prob")
    return inputs,output,lr,keep_prob

#Preprocessing the targets
def preprocess_targets(output,questionswordstoint,batch_size):
    left_side=tf.fill([batch_size,1],questionswordstoint["<SOS>"])
    right_side=tf.strided_slice(output,[0,0],[batch_size,-1],[1,1])
...
#Building the decoder model for testing/validation
